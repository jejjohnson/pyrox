"""Tests for `pyrox_nn.preprocessing._pandas`."""
...
from pyrox_nn.preprocessing import (
    SpatiotemporalFit,
    encode_time_column,
    fit_spatiotemporal,
    fit_standardization,
...
def test_pandas_isolation():
    """`pyrox_nn` must not import pandas (layers are pandas-free by design).

    `pyrox_nn.api._bnf` does import pandas, since the estimator facade
    takes pandas DataFrames — that's intentional. We only enforce the
    isolation on `pyrox_nn`.
    """
    import ast
    import pathlib
