...
`pyrox_gp.MarkovGPPrior`: it wraps an `SDEKernel` and a
...
The class duck-types `pyrox_gp.SparseGPPrior` so it composes
with the existing variational guides (`pyrox_gp.FullRankGuide`,
`pyrox_gp.MeanFieldGuide`, `pyrox_gp.WhitenedGuide`) and
the `pyrox_gp.svgp_elbo` / `pyrox_gp.svgp_factor`
infrastructure.
...
`pyrox_gp._markov` is asymptotically cheaper, but is not yet
wired in here.
"""
