...
These were originally defined in ``pyrox.nn._layers`` /
``pyrox.nn._conditioning`` / etc., but with the package split the
deterministic forwards now live in ``geonnax``. ``pyrox.nn`` keeps the
names available so existing user imports
(``from pyrox.nn import SIREN``) keep working.
"""
